...
The Cabbages and Kings is available as Project Gutenberg EBook #2777. We first download the text and strip off the Project Gutenberg header and footer.

```{r }
url <- "http://www.gutenberg.org/cache/epub/2777/pg2777.txt"
raw <- readLines(url, encoding = "UTF-8")

# the text starts after the Project Gutenberg header...
start <- grep("^CABBAGES AND KINGS", raw) -1
stop <- grep("^\\*\\*\\*END OF THE PROJECT GUTENBERG EBOOK", raw) + 1
...
#half_title <- grep("^CABBAGES AND KINGS", lines)

# chapters start with roman numerals, etc...
chapter <- grep("^[MDCLXVI]+$", lines)
...
```{r}
text_tokens(data[1]) # Chapter 24's tokens
```
...
The analysis tells us that Heap’s law accurately characterizes the lexical diversity (type-to-token ratio) for the chapters in Cabbages and Kings. The number of unique types grows roughly as the number of tokens raised to the power 0.6.
